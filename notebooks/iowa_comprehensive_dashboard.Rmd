...
# Load project utilities (includes normalize, theme_iowa, safe_read_csv, etc.)
source(here("scripts/utils.R"))

# Helper function to load data with fallback
load_iowa_data <- function(name) {
  processed_path <- here("data/processed", paste0("iowa_", name, "_analyzed.csv"))
  raw_path <- here("data/raw", paste0("iowa_", name, "_data.csv"))
  safe_read_csv(processed_path, fallback_path = raw_path)
}
```
...
# Load all datasets using helper function
cities <- safe_read_csv(
  here("data/processed/iowa_cities_analyzed.csv"),
  fallback_path = here("data/raw/iowa_major_cities.csv")
)
economic <- load_iowa_data("economic")
housing <- load_iowa_data("housing")
education <- load_iowa_data("education")
crime <- load_iowa_data("crime")

# Report what was loaded
datasets_loaded <- sum(!sapply(list(cities, economic, housing, education, crime), is.null))
```
...
if(!is.null(economic) && "major_industry" %in% names(economic)) {
  p <- economic %>%
    ...
    theme_iowa(legend_position = "none")
